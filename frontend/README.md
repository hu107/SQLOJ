...
表单包含标题（最多 200 个字符）、难度、状态、题目描述、标准答案 SQL 和结果顺序。必填内容与枚举值在提交前校验，SQL 原样提交，保存失败保留输入。保存中禁止重复提交和关闭窗口。编辑、删除成功后保留当前分页；删除空末页时自动回退。后端按 ID 升序返回列表，因此新增成功后根据最新总数进入末页查看新题目。
